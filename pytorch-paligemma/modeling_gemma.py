...
class GemmaForCausalLM(nn.Module):
    """
    A simple wrapper around the GemmaModel thant adds a language model head.
    """
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        logits = logits.float() # Cast to float to avoid issues with mixed precison training
...
class PaliGemmaMultiModalProjector(nn.Module):
    """
    A simple linear layer that projects the image feature to the same as the text embedding dimension.
    """
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def tie_weights(self): 
        """
        Ties the weights of the input embedding layer and the output projection layer 
        in the language model to reduce the number of parameters and improve training efficiency.

        Returns:
            Result of the language model's internal tie_weights() method.
        """
    ...
    def _merge_input_ids_with_image_features(
        ...
        """
            This section rquires further reading to understand.
        """
    ...
    def forward(
        ...
        # Merge the embeddings of text tokens and the image tokens
